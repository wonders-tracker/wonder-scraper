...
def generate_txt_report(data: dict) -> str:
    ...
    lines.append("|" + f"  {data['period_start'].strftime('%b %d')} - {data['period_end'].strftime('%b %d, %Y')}  ".center(72) + "|")
    lines.append("+" + "=" * 72 + "+")
    ...
    lines.append("=" * 74)
    lines.append(f"  Total Sales:      {s['total_sales']:,} transactions    ({'+' if s['sales_change_pct'] >= 0 else ''}{s['sales_change_pct']:.1f}% vs prev period)")
    lines.append(f"  Total Volume:     {format_currency(s['total_volume']):>12}       ({'+' if s['volume_change_pct'] >= 0 else ''}{s['volume_change_pct']:.1f}% vs prev period)")
    ...
    if data["daily"]:
        ...
        for d in data["daily"]:
            ...
            lines.append(f"  {d['date'].strftime('%m/%d')} {day_name} | {bar_txt(d['volume'], max_vol)} | {d['sales']:2} sales | {format_currency(d['volume']):>10}")
    ...
    if data["by_type"]:
        ...
        for t in data["by_type"]:
            ...
            lines.append(f"  {t['type']:8} | {bar_txt(t['volume'], max_vol)} | {t['sales']:2} sales | {format_currency(t['volume']):>10} ({pct:4.1f}%)")
    ...
    if data["top_volume"]:
        ...
        for i, t in enumerate(data["top_volume"], 1):
            name = t["name"][:28]
            lines.append(f"  {i:2}. {name:28} | {bar_txt(t['volume'], max_vol, 20)} | {t['sales']:2}x | {format_currency(t['volume']):>10}")
    ...
    if data["gainers"]:
        for g in data["gainers"]:
            lines.append(f"    [+] {g['name'][:30]:30} | {format_currency(g['previous']):>8} -> {format_currency(g['current']):>8} | +{g['change_pct']:.1f}%")
    else:
    ...
    if data["losers"]:
        for l in data["losers"]:
            lines.append(f"    [-] {l['name'][:30]:30} | {format_currency(l['previous']):>8} -> {format_currency(l['current']):>8} | {l['change_pct']:.1f}%")
    else:
    ...
    if data["deals"]:
        for d in data["deals"]:
            lines.append(f"  [$] {d['name'][:32]:32} | {format_currency(d['sold_price']):>8} (floor: {format_currency(d['floor'])}) | {d['discount_pct']:.0f}% off")
    else:
...
def generate_md_report(data: dict) -> str:
    ...
    lines.append("|--------|-------|-------------|")
    lines.append(f"| Total Sales | {s['total_sales']:,} | {'+' if s['sales_change_pct'] >= 0 else ''}{s['sales_change_pct']:.1f}% |")
    lines.append(f"| Total Volume | {format_currency(s['total_volume'])} | {'+' if s['volume_change_pct'] >= 0 else ''}{s['volume_change_pct']:.1f}% |")
    ...
    if data["daily"]:
        ...
        for d in data["daily"]:
            ...
            lines.append(f"| {d['date'].strftime('%m/%d')} ({day_name}) | {d['sales']} | {format_currency(d['volume'])} | `{trend}` |")
        lines.append("")
    ...
    if data["top_volume"]:
        ...
        lines.append("|------|------|-------|--------|-----------|")
        for i, t in enumerate(data["top_volume"], 1):
            lines.append(f"| {i} | {t['name'][:35]} | {t['sales']} | {format_currency(t['volume'])} | {format_currency(t['avg'])} |")
        lines.append("")
    ...
    if data["gainers"]:
        ...
        lines.append("|------|----------|---------|--------|")
        for g in data["gainers"]:
            lines.append(f"| {g['name'][:35]} | {format_currency(g['previous'])} | {format_currency(g['current'])} | **+{g['change_pct']:.1f}%** |")
    else:
        lines.append("*No significant gainers with enough data*")
    lines.append("")
    ...
    if data["losers"]:
        ...
        lines.append("|------|----------|---------|--------|")
        for l in data["losers"]:
            lines.append(f"| {l['name'][:35]} | {format_currency(l['previous'])} | {format_currency(l['current'])} | **{l['change_pct']:.1f}%** |")
    else:
        lines.append("*No significant losers with enough data*")
    lines.append("")
    ...
    if data["deals"]:
        ...
        lines.append("|------|------------|-------|----------|")
        for d in data["deals"]:
            lines.append(f"| {d['name'][:35]} | {format_currency(d['sold_price'])} | {format_currency(d['floor'])} | **{d['discount_pct']:.0f}% off** |")
    else:
        lines.append("*No significant deals found this period*")
    lines.append("")
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["txt", "md", "all"],
        default="all",
        help="Output format: 'txt', 'md', or 'all'"
    )
    parser.add_argument(
        "--days", "-d",
        type=int,
        help="Custom number of days (overrides --type)"
    )
    parser.add_argument(
        "--output", "-o",
        default="data/marketReports",
        help="Output directory"
    )
    parser.add_argument(
        "--print", "-p",
        action="store_true",
        help="Also print report to terminal"
    )
