...
def fetch_all_data(days: int = 180) -> pd.DataFrame:
    ...
    df = pd.DataFrame(rows, columns=[
        'card_id', 'card_name', 'card_number', 'rarity', 'card_type',
        'set_name', 'treatment', 'price', 'sale_date', 'platform'
    ])
...
def analyze_card_types(df: pd.DataFrame):
    ...
    type_stats = paper_df.groupby('card_type').agg({
...
def analyze_volatility(df: pd.DataFrame):
    ...
    vol_df = df.groupby(['card_id', 'card_name', 'treatment']).agg({
        'price': ['median', 'std', 'count', 'min', 'max']
    }).reset_index()
...
def analyze_liquidity(df: pd.DataFrame):
    ...
    card_liquidity = df.groupby(['card_id', 'card_name', 'rarity']).agg({
        'price': ['count', 'median'],
        'treatment': 'nunique'
    }).reset_index()
...
def analyze_deals(df: pd.DataFrame):
    ...
    floors = df.groupby(['card_id', 'card_name', 'treatment']).agg({
        'price': lambda x: np.mean(sorted(x)[:4]) if len(x) >= 4 else x.min()
    }).reset_index()
    ...
    deal_freq = df_with_floor.groupby('treatment').agg({
        'is_deal': ['sum', 'mean'],
        'price': 'count'
    })
...
def analyze_card_clusters(df: pd.DataFrame):
    ...
    card_features = df.groupby(['card_id', 'card_name', 'rarity']).agg({
        'price': ['median', 'std', 'count', 'min', 'max'],
        'treatment': 'nunique'
    }).reset_index()
    card_features.columns = ['card_id', 'card_name', 'rarity', 'median_price', 'price_std',
                             'sale_count', 'min_price', 'max_price', 'treatment_count']
