...
def cleanup_bad_entries():
    ...
    with Session(engine) as session:
        ...
        sealed = session.exec(select(Card).where(Card.product_type.in_(["Box", "Pack"]))).all()
        ...
        for card in sealed:
            ...
            is_wotf = (
                "wonders" in card_name_lower
                or "existence" in card_name_lower
                or "wonders" in set_name_lower
                or "existence" in set_name_lower
            )
