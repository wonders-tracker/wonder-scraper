...
EXCLUDED_TREATMENTS = {
    'Box', 'Dragon Box', 'Play Bundle', 'Silver Pack', 'Collector Bundle',
    'Collector Booster Pack', 'Play Booster Pack', 'Collector Booster Box',
    'Booster Pack', 'Booster Box', 'Bundle', 'Pack'
}

# Core treatments we care about
CORE_TREATMENTS = {
    'Classic Paper', 'Classic Foil', 'Stonefoil', 'Formless Foil',
    'Serialized', 'OCM Serialized', 'Alt Art', 'Character Proof'
}
...
def fetch_sales_data(days: int = 180) -> pd.DataFrame:
    ...
    df = pd.DataFrame(rows, columns=[
        ...
        'treatment', 'price', 'sale_date', 'platform'
    ])
...
def add_ar_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['floor_4'] = df.groupby(group_cols)['price'].transform(
        lambda x: x.shift(1).rolling(4, min_periods=1).min()
    )
...
def add_order_book_features(sales_df: pd.DataFrame, listings_df: pd.DataFrame) -> pd.DataFrame:
    ...
    sales_df = sales_df.merge(
        listings_df[['card_id', 'treatment', 'listing_count', 'lowest_ask']],
...
FEATURE_COLS = [
    ...
    'rel_lag1', 'rel_roll_mean', 'rel_roll_min',

    # AR features (absolute)
    'price_lag1', 'floor_4',

    # Treatment
    'treatment_mult', 'treatment_code',

    # Liquidity
    'illiquidity', 'card_sales_count', 'treatment_sales_count',

    # Order book
    'listing_count', 'has_listings', 'ask_relative', 'ask_vs_floor',
]
...
def print_results(metrics: dict, model, test_df: pd.DataFrame):
    ...
    treatment_stats = test_df.groupby('treatment').agg({
        'abs_error': 'mean',
        'pct_error': 'median',
        'price': 'count'
    }).round(2)
    ...
    test_df['price_bucket'] = pd.cut(test_df['price'],
                                      bins=[0, 10, 25, 50, 100, 2000],
                                      labels=['<$10', '$10-25', '$25-50', '$50-100', '>$100'])
    print(f"\nPerformance by Price Range:")
    price_stats = test_df.groupby('price_bucket', observed=True).agg({
        'abs_error': 'mean',
        'pct_error': 'median',
        'price': 'count'
    }).round(2)
