...
class TestDataQualityIntegration:
    ...
    def test_sealed_boxes_have_sealed_treatment(self, integration_session):
        ...
        assert len(issues) < len(boxes) * 0.1, "More than 10% of eBay boxes have unexpected treatments"
    ...
    def test_singles_have_card_treatments(self, integration_session):
        ...
        valid_single_treatments = {
            ...
            "Stonefoil Alt Art",
            "Formless Foil Alt Art",
            "OCM Serialized Alt Art",
            "Prerelease Alt Art",
            "Promo Alt Art",
            "Proof/Sample Alt Art",
            "Error/Errata Alt Art",
            "",  # Allow empty/null
        }
        ...
        assert len(issues) < len(singles) * 0.05, "More than 5% of singles have unexpected treatments"
    ...
    def test_packs_have_subtypes(self, integration_session):
        ...
        packs = integration_session.exec(
            select(MarketPrice).join(Card, MarketPrice.card_id == Card.id).where(Card.product_type == "Pack").limit(50)
        ).all()
    ...
    def test_grading_detected_correctly(self, integration_session):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.platform == "ebay").where(MarketPrice.grading.is_(None)).limit(500)
        ).all()
    ...
    def test_quantity_detected_correctly(self, integration_session):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.platform == "ebay").where(MarketPrice.quantity == 1).limit(500)
        ).all()
        ...
        assert len(issues) < len(listings) * 0.02, f"{len(issues)} listings may have incorrect quantity"
    ...
    def test_no_wrong_game_cards(self, integration_session):
        ...
        listings = integration_session.exec(select(MarketPrice).where(MarketPrice.platform == "ebay").limit(500)).all()
...
class TestCardNameNormalization:
    ...
    def test_spelling_corrections(self, typo, correction):
        ...
        from scripts.cleanup_listing_data import apply_spelling_corrections
