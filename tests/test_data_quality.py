...
class DataQualityReport:
    ...
    def add_issue(self, category: str, severity: str, record_id: int, description: str, details: Dict[str, Any] = None):
        self.issues.append(
            {
                "category": category,
...
class TestPlatformURLConsistency:
    ...
    def test_opensea_listings_have_opensea_urls(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.platform == "opensea", MarketPrice.url.isnot(None))
        ).all()
        ...
        for listing in listings:
            if listing.url and "opensea.io" not in listing.url:
                quality_report.add_issue(
                    ...
                    description="OpenSea listing has non-OpenSea URL",
    ...
    def test_ebay_listings_have_ebay_urls(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.platform == "ebay", MarketPrice.url.isnot(None))
        ).all()
        ...
        for listing in listings:
            if listing.url and "ebay.com" not in listing.url and "ebay." not in listing.url:
                quality_report.add_issue(
                    ...
                    severity="error",
                    record_id=listing.id,
                    description="eBay listing has non-eBay URL",
                    details={"url": listing.url, "platform": listing.platform},
                )
            ...
            if listing.url:
                ...
                if "/assets/" in listing.url and "/item/" not in listing.url:
                    quality_report.add_issue(
                        ...
                        severity="warning",
                        record_id=listing.id,
                        description="OpenSea URL uses old /assets/ format instead of /item/",
...
class TestNFTTreatmentClassification:
    ...
    def test_opensea_nfts_not_using_sealed_treatment(
        self, integration_session: Session, quality_report: DataQualityReport
    ):
        """OpenSea NFT listings should not use 'Sealed' treatment (that's for physical boxes)."""
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.platform == "opensea", MarketPrice.treatment == "Sealed")
        ).all()
        ...
        for listing in listings:
            quality_report.add_issue(
                ...
                severity="error",
                record_id=listing.id,
                description="OpenSea NFT using 'Sealed' treatment (should be NFT trait)",
                details={"title": listing.title, "treatment": listing.treatment, "price": listing.price},
            )
    ...
    def test_opensea_nfts_not_using_physical_treatments(
        self, integration_session: Session, quality_report: DataQualityReport
    ):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.platform == "opensea", MarketPrice.treatment.in_(physical_treatments))
        ).all()
        ...
        for listing in listings:
            quality_report.add_issue(
                ...
                severity="warning",
                record_id=listing.id,
                description="OpenSea NFT using physical card treatment",
                details={
                    "title": listing.title,
    ...
    def test_opensea_nfts_not_using_generic_nft_treatment(
        self, integration_session: Session, quality_report: DataQualityReport
    ):
        """OpenSea NFTs should use actual traits, not generic 'NFT' treatment."""
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.platform == "opensea", MarketPrice.treatment == "NFT")
        ).all()
        ...
        for listing in listings:
            quality_report.add_issue(
                ...
                severity="warning",
                record_id=listing.id,
                description="OpenSea NFT using generic 'NFT' treatment instead of traits",
                details={"title": listing.title, "treatment": listing.treatment, "card_id": listing.card_id},
            )
    ...
    def test_ebay_boxes_using_sealed_treatment(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        box_cards = integration_session.exec(select(Card).where(Card.product_type == "Box")).all()
        ...
        for listing in listings:
            ...
            if listing.treatment in ["NFT", "Rare", "Epic", "Legendary", "Common", "Uncommon"]:
                quality_report.add_issue(
                    ...
                    severity="warning",
                    record_id=listing.id,
                    description="eBay box listing with NFT-style treatment",
                    details={
                        "title": listing.title,
                        "treatment": listing.treatment,
                    },
...
class TestMissingRequiredFields:
    ...
    def test_sold_listings_have_prices(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.listing_type == "sold", MarketPrice.price <= 0)
        ).all()
        ...
        for listing in listings:
            quality_report.add_issue(
                ...
                severity="error",
                record_id=listing.id,
                description="Sold listing with zero or negative price",
    ...
    def test_opensea_listings_have_external_ids(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(
                MarketPrice.platform == "opensea", MarketPrice.listing_type == "sold", MarketPrice.external_id.is_(None)
            )
        ).all()
        ...
        for listing in listings:
            quality_report.add_issue(
                ...
                severity="warning",
                record_id=listing.id,
                description="OpenSea listing missing external_id (tx_hash)",
    ...
    def test_listings_have_titles(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where((MarketPrice.title.is_(None)) | (MarketPrice.title == ""))
        ).all()
        ...
        for listing in listings:
            quality_report.add_issue(
                ...
                severity="warning",
                record_id=listing.id,
                description="Listing missing title",
...
class TestDuplicateDetection:
    ...
    def test_no_duplicate_opensea_transactions(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        duplicates = integration_session.exec(
            ...
            .where(
                MarketPrice.platform == "opensea", MarketPrice.external_id.isnot(None), MarketPrice.external_id != ""
            )
        ...
        for external_id, count in duplicates:
            quality_report.add_issue(
                ...
                severity="error",
                record_id=0,
                description="Duplicate OpenSea transaction found",
    ...
    def test_no_duplicate_ebay_items(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        for external_id, count in duplicates:
            quality_report.add_issue(
                ...
                record_id=0,
                description="Duplicate eBay item found",
                details={"external_id": external_id, "count": count},
            )
...
class TestPriceAnomalies:
    ...
    def test_no_extremely_high_prices(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.listing_type == "sold", MarketPrice.price > threshold)
        ).all()
        ...
        for listing in listings:
            quality_report.add_issue(
                ...
                details={"title": listing.title, "price": listing.price, "platform": listing.platform},
            )
    ...
    def test_opensea_eth_conversion_reasonable(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        for listing in listings:
            ...
            quality_report.add_issue(
                ...
                severity=severity,
                record_id=listing.id,
                description="OpenSea price outside normal range",
                details={
                    "title": listing.title,
...
class TestCardAssociation:
    ...
    def test_opensea_proofs_linked_to_proof_cards(
        self, integration_session: Session, quality_report: DataQualityReport
    ):
        """OpenSea Character Proofs should be linked to Proof-type cards."""
        # Get proof cards
        proof_cards = integration_session.exec(select(Card).where(Card.product_type == "Proof")).all()
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.platform == "opensea", MarketPrice.card_id.notin_(proof_card_ids))
        ).all()
        ...
        for listing in listings:
            if listing.title and "proof" in listing.title.lower():
                quality_report.add_issue(
                    ...
                    severity="warning",
                    record_id=listing.id,
                    description="OpenSea 'proof' listing not linked to Proof card",
                    details={
                        "title": listing.title,
...
class TestNonWOTFContamination:
    ...
    WOTF_SETS = ["Wonders of the First", "Existence", "Genesis", "WOTF", "WotF", "Wonder"]
    ...
    def test_no_yugioh_cards_in_database(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        listings = integration_session.exec(select(MarketPrice).where(MarketPrice.listing_type == "sold")).all()
            ...
            for indicator in yugioh_indicators:
                if indicator.lower() in title_lower:
                    quality_report.add_issue(
                        ...
                        severity="error",
                        record_id=listing.id,
                        description="Yu-Gi-Oh card in WOTF database",
                        details={
                            "title": listing.title,
                            "indicator": indicator,
                            "price": listing.price,
            ...
            for indicator in pokemon_indicators:
                if indicator in title_lower:
                    quality_report.add_issue(
                        ...
                        severity="error",
                        record_id=listing.id,
                        description="Pokemon card in WOTF database",
                        details={"title": listing.title, "indicator": indicator, "card_id": listing.card_id},
                    )
                    break
            ...
            for indicator in mtg_indicators:
                if indicator in title_lower:
                    quality_report.add_issue(
                        ...
                        severity="error",
                        record_id=listing.id,
                        description="MTG card in WOTF database",
                        details={"title": listing.title, "indicator": indicator, "card_id": listing.card_id},
                    )
                    break
            ...
            for indicator in dragonball_indicators:
                if indicator in title_lower:
                    quality_report.add_issue(
                        ...
                        severity="error",
                        record_id=listing.id,
                        description="Dragon Ball card in WOTF database",
                        details={"title": listing.title, "indicator": indicator, "card_id": listing.card_id},
                    )
                    break
            ...
            for indicator in one_piece_indicators:
                if indicator in title_lower:
                    quality_report.add_issue(
                        ...
                        severity="error",
                        record_id=listing.id,
                        description="One Piece card in WOTF database",
                        details={"title": listing.title, "indicator": indicator, "card_id": listing.card_id},
                    )
                    break
    ...
    def test_no_flesh_and_blood_cards_in_database(
        self, integration_session: Session, quality_report: DataQualityReport
    ):
            ...
            for indicator in fab_indicators:
                if indicator in title_lower:
                    quality_report.add_issue(
                        ...
                        severity="error",
                        record_id=listing.id,
                        description="Flesh and Blood card in WOTF database",
                        details={"title": listing.title, "indicator": indicator, "card_id": listing.card_id},
                    )
                    break
            ...
            for indicator in lorcana_indicators:
                if indicator in title_lower:
                    quality_report.add_issue(
                        ...
                        severity="error",
                        record_id=listing.id,
                        description="Disney Lorcana card in WOTF database",
                        details={"title": listing.title, "indicator": indicator, "card_id": listing.card_id},
                    )
                    break
            ...
            for indicator in weiss_indicators:
                if indicator in title_lower:
                    quality_report.add_issue(
                        ...
                        severity="error",
                        record_id=listing.id,
                        description="Weiss Schwarz card in WOTF database",
                        details={"title": listing.title, "indicator": indicator, "card_id": listing.card_id},
                    )
                    break
    ...
    def test_no_digimon_cards_in_database(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        digimon_indicators = [
            ...
            "great legend",
            "booster set digimon",
            "starter deck digimon",
            "release special",
            "alternate art digimon",
            "sec rare",
            "sr digimon",
        ]
            ...
            for indicator in digimon_indicators:
                if indicator in title_lower:
                    quality_report.add_issue(
                        ...
                        severity="error",
                        record_id=listing.id,
                        description="Digimon card in WOTF database",
                        details={"title": listing.title, "indicator": indicator, "card_id": listing.card_id},
                    )
                    break
...
class TestTreatmentExtraction:
    ...
    def test_stone_foil_extraction(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.listing_type == "sold", MarketPrice.platform == "ebay")
        ).all()
        ...
        for listing in listings:
            ...
            if "stone" in title_lower and "stone" not in treatment_lower:
                quality_report.add_issue(
                    ...
                    severity="warning",
                    record_id=listing.id,
                    description="Stone Foil not extracted from title",
                    details={
                        "title": listing.title,
        ...
        for listing in listings:
            ...
            if foil_in_title and not foil_in_treatment:
                quality_report.add_issue(
                    ...
                    severity="warning",
                    record_id=listing.id,
                    description="Foil not extracted from title",
                    details={
                        "title": listing.title,
                        "current_treatment": listing.treatment,
                    },
        ...
        for listing in listings:
            ...
            if has_serial_in_title and not has_serial_in_treatment:
                quality_report.add_issue(
                    ...
                    severity="warning",
                    record_id=listing.id,
                    description="Serialized not extracted from title",
                    details={
                        "title": listing.title,
                        "current_treatment": listing.treatment,
                    },
...
class TestSpecificCardIssues:
    ...
    def test_progo_data_quality(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        # Find Progo card
        progo = integration_session.exec(select(Card).where(Card.name.ilike("%progo%"))).first()

        if not progo:
            pytest.skip("Progo card not found in database")

        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.card_id == progo.id, MarketPrice.listing_type == "sold")
        ).all()
        ...
        for listing in listings:
            ...
            if any(ind in title_lower for ind in ["yu-gi-oh", "pokemon", "mtg", "magic"]):
                quality_report.add_issue(
                    ...
                    severity="error",
                    record_id=listing.id,
                    description="Non-WOTF card in Progo listings",
            ...
            if "wotf" not in title_lower and "wonder" not in title_lower and "progo" not in title_lower:
                quality_report.add_issue(
                    ...
                    severity="warning",
                    record_id=listing.id,
                    description="Progo listing missing WOTF indicators",
    ...
    def test_deep_black_goop_treatment(self, integration_session: Session, quality_report: DataQualityReport):
        ...
        goop = integration_session.exec(select(Card).where(Card.name.ilike("%deep black goop%"))).first()

        if not goop:
            pytest.skip("Deep Black Goop card not found in database")

        listings = integration_session.exec(
            select(MarketPrice).where(MarketPrice.card_id == goop.id, MarketPrice.listing_type == "sold")
        ).all()
            ...
            if "stone" in title_lower:
                ...
                if "stone" not in treatment_lower:
                    ...
                    quality_report.add_issue(
                        ...
                        severity="warning",
                        record_id=listing.id,
                        description="Deep Black Goop Stone Foil not extracted",
                        details={"title": listing.title, "current_treatment": listing.treatment},
                    )
...
class TestDataQualitySummary:
    ...
    def test_generate_quality_report(self, integration_session: Session):
        ...
        # Count totals
        total_listings = integration_session.exec(select(func.count(MarketPrice.id))).one()
        ...
        no_treatment = integration_session.exec(
            select(func.count(MarketPrice.id)).where((MarketPrice.treatment.is_(None)) | (MarketPrice.treatment == ""))
        ).one()

        no_url = integration_session.exec(
            select(func.count(MarketPrice.id)).where(MarketPrice.url.is_(None), MarketPrice.listing_type == "sold")
        ).one()

        generic_nft_treatment = integration_session.exec(
            select(func.count(MarketPrice.id)).where(MarketPrice.platform == "opensea", MarketPrice.treatment == "NFT")
        ).one()
